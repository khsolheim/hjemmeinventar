...
import { Brain, Sparkles, Wand2, Search, Target, MapPin, Tag, Copy, AlertTriangle, Lightbulb, TrendingUp, Zap, TestTube, BookOpen, Loader2, CheckCircle, Send } from 'lucide-react'
import { AIInsightsDashboard } from '@/components/ai/AIInsightsDashboard'
import { SmartSuggestions } from '@/components/ai/SmartSuggestions'
import { trpc } from '@/lib/trpc/client'
import { toast } from 'sonner'

export function AIPageClient({ aiEnabled }: { aiEnabled: boolean }) {
  const [testItemName, setTestItemName] = useState('')
  const [testDescription, setTestDescription] = useState('')
  const [searchQuery, setSearchQuery] = useState('')
  const [enhancedQuery, setEnhancedQuery] = useState('')

  // Ikke hent ai.isEnabled her; bruk prop for stabil SSR
  const isEnabled = aiEnabled
  ...
  const handleSearchEnhancement = () => {
    if (searchQuery.trim()) {
      enhanceSearch.mutate({ query: searchQuery.trim(), context: 'Inventarsøk' })
    }
  }

  return (
    <div className="page container mx-auto px-4 py-8 max-w-6xl cq">
      <div className="text-center mb-8" style={{ minHeight: '18cqh' }}>
        <div className="flex items-center justify-center gap-3 mb-4">
          <Brain className="w-8 h-8 text-blue-600" />
          <h1 className="text-3xl font-bold">AI-Funksjoner</h1>
          <Badge variant="default" className={isEnabled ? 'bg-green-600' : 'bg-gray-400'}>
            <Sparkles className="w-3 h-3 mr-1" />
            {isEnabled ? 'Aktivt' : 'Ikke aktivert'}
          </Badge>
        </div>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Intelligente features drevet av OpenAI som gjør inventarhåndtering smartere og mer effektiv
        </p>
        <div className="max-w-2xl mx-auto mt-4" style={{ minHeight: '64px' }}>
          {!isEnabled ? (
            <Alert>
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription className="text-center">
                <strong>AI-funksjoner er ikke aktivert.</strong><br />
                Legg til din OpenAI API-nøkkel i miljøvariablene for å aktivere funksjonene.
              </AlertDescription>
            </Alert>
          ) : (
      ...
      <Tabs defaultValue="dashboard" className="space-y-6 cq">
        <TabsList className="grid w-full grid-cols-4" style={{ minHeight: '48px' }}>
        ...
        <TabsContent value="dashboard" className="min-h-[720px]">
          <div className="min-h-[700px]">
            <AIInsightsDashboard aiEnabled={isEnabled} />
          </div>
        </TabsContent>

        <TabsContent value="suggestions" className="hidden data-[state=active]:block space-y-6 min-h-[480px]">
          <Card>
            <CardHeader>
        ...
        <TabsContent value="tools" className="hidden data-[state=active]:block space-y-6 min-h-[480px]">
                  ...
                  <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => navigator.clipboard.writeText(enhancedQuery)}
                      >
              ...
              <CardContent>
                <div className="space-y-3 text-center text-muted-foreground">
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm font-medium">Optimaliseringsråd</p>
                    <p className="text-xs">Få personlige tips basert på dine vaner</p>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="learn" className="hidden data-[state=active]:block space-y-6 min-h-[480px]">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
