        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button variant="outline" onClick={() => {
              ...
              const html = `<!DOCTYPE html><html><head><title>Etiketter</title><style>body{margin:0;padding:12px;font-family:Arial} .wrap{display:flex;flex-wrap:wrap;gap:8px} .box{border:1px solid #000; padding:8px; width:280px; margin:4px} .title{font-weight:bold; font-size:14px; margin:6px 0} .small{font-size:12px; color:#444} .code{font-family:monospace; font-size:12px}</style></head><body><div id='wrap' class='wrap'></div><script>window.__LABELS__=${JSON.stringify(labels)};window.__LOGO__=${JSON.stringify(logo||'')};window.__EXTRA1__=${JSON.stringify(extra1||'')};window.__EXTRA2__=${JSON.stringify(extra2||'')};window.__SHOWURL__=${JSON.stringify(!!showUrl)};window.addEventListener('load',function(){try{var wrap=document.getElementById('wrap');(window.__LABELS__||[]).forEach(function(l,i){var div=document.createElement('div');div.className='box';div.innerHTML=(window.__LOGO__?"<img src='"+window.__LOGO__+"' style='max-width:260px;max-height:40px'/>":"")+"<div class='title'>"+l.itemName+"</div><div class='small'>"+l.locationName+"</div>"+(window.__EXTRA1__?"<div class='small'>"+window.__EXTRA1__+"</div>":"")+(window.__EXTRA2__?"<div class='small'>"+window.__EXTRA2__+"</div>":"")+"<img id='qr-"+i+"' style='width:160px;height:160px'/><div class='code'>"+l.qrCode+"</div>"+(window.__SHOWURL__?"<div class='small'>"+l.url+"</div>":"");wrap.appendChild(div);var canvas=document.createElement('canvas');canvas.width=160;canvas.height=160;var ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,160,160);ctx.fillStyle='#000';var hash=0;for(var j=0;j<l.url.length;j++){hash=((hash<<5)-hash)+l.url.charCodeAt(j);hash=hash&hash;}hash=Math.abs(hash);for(var y=0;y<20;y++){for(var x=0;x<20;x++){var pixelHash=Math.abs(hash+x+y);if(pixelHash%3===0){ctx.fillRect(x*8,y*8,8,8);}}}var img=document.getElementById('qr-"+i+"');if(img)img.src=canvas.toDataURL();});}catch(e){console.error('QR generation failed:',e);}});</script></body></html>`
