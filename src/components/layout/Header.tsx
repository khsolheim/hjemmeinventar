        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button variant="outline" onClick={() => {
              ...
              const html = `<!DOCTYPE html><html><head><title>Etiketter</title><style>body{margin:0;padding:12px;font-family:Arial} .wrap{display:flex;flex-wrap:wrap;gap:8px} .box{border:1px solid #000; padding:8px; width:280px; margin:4px} .title{font-weight:bold; font-size:14px; margin:6px 0} .small{font-size:12px; color:#444} .code{font-family:monospace; font-size:12px}</style></head><body><script src='https://unpkg.com/qrcode/build/qrcode.min.js'></script><div id='wrap' class='wrap'></div><script>window.__LABELS__=${JSON.stringify(labels)};window.__LOGO__=${JSON.stringify(logo||'')};window.__EXTRA1__=${JSON.stringify(extra1||'')};window.__EXTRA2__=${JSON.stringify(extra2||'')};window.__SHOWURL__=${JSON.stringify(!!showUrl)};window.addEventListener('load',function(){var wrap=document.getElementById('wrap');(window.__LABELS__||[]).forEach(function(l,i){var div=document.createElement('div');div.className='box';div.innerHTML=(window.__LOGO__?"<img src='"+window.__LOGO__+"' style='max-width:260px;max-height:40px'/>":"")+"<div class='title'>"+l.itemName+"</div><div class='small'>"+l.locationName+"</div>"+(window.__EXTRA1__?"<div class='small'>"+window.__EXTRA1__+"</div>":"")+(window.__EXTRA2__?"<div class='small'>"+window.__EXTRA2__+"</div>":"")+"<img id='qr-"+i+"' style='width:160px;height:160px'/><div class='code'>"+l.qrCode+"</div>"+(window.__SHOWURL__?"<div class='small'>"+l.url+"</div>":"");wrap.appendChild(div);window.QRCode.toDataURL(l.url,{width:160,margin:1},function(err,data){if(!err){var img=document.getElementById('qr-"+i+"');if(img) img.src=data;}});});});</script></body></html>`
